     ...
     final prefs = await SharedPreferences.getInstance();
      ...
      Future<void> _sendMessage() async {
        if (_controller.text.trim().isNotEmpty && !_isThinkingNotifier.value) {
            ...
            final String title = _isNewChat ? userMessage : _currentChatTitleNotifier.value!; // add title if its a new chat
              _messagesNotifier.value = [
                 ..._messagesNotifier.value,
                  ChatMessage(sender: 'user', text: userMessage, timeStamp: DateTime.now().toString()),
                 ];
                _controller.clear();
             try {
                  final response = await http.post(
                        Uri.parse('https://devred.pythonanywhere.com/api/chat/${_farmProfile?.id}/'), // added the farm id
                       headers: {
                          'Authorization': 'Token $_token',
                           'Content-Type': 'application/json',
                         },
                         body: json.encode({'message': userMessage, 'chat_title': title}), //send the user message
                     );
                     if (response.statusCode == 200) {
      ...
      void _handleDrawerItemTap(int chatHistoryId) async {
           try {
              final response = await http.get(
                  Uri.parse('https://devred.pythonanywhere.com/api/chat/$chatHistoryId/'), // send the id of the selected chat history
                   headers: {
                     'Authorization': 'Token $_token',
                   },
                );
